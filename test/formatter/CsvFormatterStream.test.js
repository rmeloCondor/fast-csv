...
const CsvFormatterStream = require('../../lib/formatter/CsvFormatterStream');
const FormatterOptions = require('../../lib/formatter/FormatterOptions');
const { RecordingStream } = require('../helpers');
...
describe('CsvFormatterStream', () => {
    ...
    const pipeToRecordingStream = (formatter, rows) => new Promise((res, rej) => {
        ...
        rows.forEach(row => formatter.write(row, null));
        formatter.end();
    });

    const formatRows = (rows, options = {}) => pipeToRecordingStream(csv.format(options), rows);

    it('should throw an error if the constructor is called with null', () => {
        assert.throws(() => new CsvFormatterStream(null), /TypeError: formatterOptions is required/);
    });

    it('should throw an error if the constructor is called with a plain object', () => {
        assert.throws(() => new CsvFormatterStream({}), /TypeError: formatterOptions is required/);
    });
    ...
    describe('#transform', () => {
        it('should support transforming an array of arrays', () => {
            const formatter = new CsvFormatterStream(new FormatterOptions({ headers: true }))
                .transform(row => row.map(entry => entry.toUpperCase()));
        ...
        it('should support transforming an array of multi-dimensional arrays', () => {
            const formatter = new CsvFormatterStream(new FormatterOptions({ headers: true }))
                .transform(row => row.map(entry => [ entry[0], entry[1].toUpperCase() ]));
    ...
    describe('.writeToPath', () => {
        const writeToPath = (rows, options = {}) => new Promise((res, rej) => {
    ...
    describe('.writeToStream', () => {
        const writeToStream = (rows, options = {}) => new Promise((res, rej) => {
            const rs = new RecordingStream();
